...
**Per-round candidates:** reuse `resolveRoundCandidates(group)` unchanged (already handles manual = stored order, random = shuffled sample of `sampleSize`) — this array's order *is* the one-at-a-time presentation queue; the current item is `candidates[placedCount]`.
