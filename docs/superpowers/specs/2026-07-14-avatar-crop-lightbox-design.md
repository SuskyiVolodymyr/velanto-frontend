...
- Cropping produces a square image `File` that goes through the **existing** `uploadMedia(file, "avatar")` → `usersClient.setAvatar(key)` flow (`useUpdateAvatar`). The backend still re-encodes to 256² WebP — cropping just controls *which* square it keeps.
...
- Picking a file no longer uploads immediately. After the existing client validation (image type + ≤1 MB on the *original* pick), it stores the file in state and opens `AvatarCropModal`.
