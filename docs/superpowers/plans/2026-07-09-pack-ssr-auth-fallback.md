...
**Full design rationale:** `docs/superpowers/specs/2026-07-09-pack-ssr-auth-fallback-design.md` — read it once before starting if anything below is unclear on *why*, not just *what*.
...
3. Log out, log in as a *different* plain `user`-role account (not the owner, not moderator+), and confirm the same pending pack's `/packs/[id]` URL now correctly 404s (no over-broad disclosure).
