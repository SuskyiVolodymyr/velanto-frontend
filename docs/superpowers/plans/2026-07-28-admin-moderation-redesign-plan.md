...
_small_ one: it is the third instance of an interaction this codebase
...
log keyed to a single pack (the _admin-wide_ Logs tab is a different,
...
its pinned `itemIds` in order, which _is_ exactly what was shown (per the
...
to _separate_ URLs, so no mode-switcher chrome is needed, just the
