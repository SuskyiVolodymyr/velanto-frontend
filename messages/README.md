...
- **"pack"** = a quiz/elimination content pack a user creates. Translate the concept consistently (package/bundle, or an accepted transliteration like hi `पैक` / bn `প্যাক` / ur `پیک`). Never translate the actual user-authored pack *content* — only UI chrome.
