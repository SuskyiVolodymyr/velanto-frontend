...
describe("ModerationQueueScreen", () => {
  ...
  it("renders the queue list for a moderator user, showing title and format", async () => {
    ...
    render(<ModerationQueueScreen />);
    await waitFor(() => expect(screen.getByText("Best Pack")).toBeInTheDocument());
  ...
  it("shows the empty state when the queue is empty", async () => {
    mockAuth("moderator");
    mockedPacksClient.moderationQueue.mockResolvedValue({ items: [], total: 0, page: 1, limit: 20 });
    render(<ModerationQueueScreen />);
  ...
  it("approves a pack and removes it from the list on success", async () => {
    mockAuth("moderator");
    mockedPacksClient.moderationQueue.mockResolvedValue({ items: [makePack()], total: 1, page: 1, limit: 20 });
    mockedPacksClient.approve.mockResolvedValue(makePack({ status: "approved" }));
    render(<ModerationQueueScreen />);
    await waitFor(() => expect(screen.getByText("Best Pack")).toBeInTheDocument());
    await userEvent.click(screen.getByRole("button", { name: "Approve" }));
    expect(mockedPacksClient.approve).toHaveBeenCalledWith("p1");
    await waitFor(() => expect(screen.queryByText("Best Pack")).not.toBeInTheDocument());
  });

  it("rejects a pack with a typed reason and removes it from the list on success", async () => {
    mockAuth("moderator");
    mockedPacksClient.moderationQueue.mockResolvedValue({ items: [makePack()], total: 1, page: 1, limit: 20 });
    mockedPacksClient.reject.mockResolvedValue(makePack({ status: "rejected" }));
    render(<ModerationQueueScreen />);
    await waitFor(() => expect(screen.getByText("Best Pack")).toBeInTheDocument());
    ...
    expect(mockedPacksClient.reject).toHaveBeenCalledWith("p1", "Duplicate of an existing pack");
    await waitFor(() => expect(screen.queryByText("Best Pack")).not.toBeInTheDocument());
  });
  ...
  it("shows a per-row error and keeps the row when approve fails", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Best Pack")).toBeInTheDocument());
    await userEvent.click(screen.getByRole("button", { name: "Approve" }));
    await waitFor(() => expect(screen.getByText(/couldn't approve this pack/i)).toBeInTheDocument());
  ...
  it("shows a per-row error and keeps the row when reject fails", async () => {
    ...
    await screen.findByLabelText("Rejection reason");
    await userEvent.click(screen.getByRole("button", { name: "Confirm reject" }));
    await waitFor(() => expect(screen.getByText(/couldn't reject this pack/i)).toBeInTheDocument());
    expect(screen.getByText("Best Pack")).toBeInTheDocument();
  });
  ...
  it("loads more results and appends without duplicates", async () => {
    ...
    mockedPacksClient.moderationQueue
      .mockResolvedValueOnce({ items: [makePack()], total: 2, page: 1, limit: 1 })
    ...
    await waitFor(() => expect(screen.getByText("Second Pack")).toBeInTheDocument());
    expect(screen.getByText("Best Pack")).toBeInTheDocument();
    expect(mockedPacksClient.moderationQueue).toHaveBeenLastCalledWith({ page: 2, limit: 20 });
  });
});
