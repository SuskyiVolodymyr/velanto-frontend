...
describe("VoteButtons", () => {
  ...
  it("renders like/dislike counts from the initial props", () => {
    mockAuth(true);
    render(<VoteButtons packId="pack-1" initialLikes={3} initialDislikes={1} initialMyVote={null} />);
  ...
  it("clicking Like calls packsClient.vote with value 1 and updates counts from the response", async () => {
    ...
    render(<VoteButtons packId="pack-1" initialLikes={0} initialDislikes={0} initialMyVote={null} />);
  ...
  it("clicking the currently-active Like button again toggles it off (still calls vote with value 1)", async () => {
    ...
    render(<VoteButtons packId="pack-1" initialLikes={1} initialDislikes={0} initialMyVote={1} />);
    ...
    expect(mockedPacksClient.vote).toHaveBeenCalledWith("pack-1", 1);
    await waitFor(() => expect(within(likeButton).getByText("0")).toBeInTheDocument());
  });
